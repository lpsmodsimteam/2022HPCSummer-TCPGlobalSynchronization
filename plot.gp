...
stats "receiver_data.csv" using 1:4 nooutput # collect statistic info

set multiplot layout 3,2 title "TCP Global Synchronization"

    set title "Queue Size"
    set ylabel "Packets in Queue"
    plot "receiver_data.csv" using 1:2 title "Queue Size of Receiver Node" with lines lw 3 

    set title "Packet Loss"
    set ylabel "Packets Dropped"
    plot "receiver_data.csv" using 1:3 title "Packet Loss" with lines lw 3

    set title "Link Utilization"
    set ylabel "Link Utilization (%)"  
    plot "receiver_data.csv" using 1:4 title "Link Utilization" with lines lw 3
    f(x) = mean_y
    fit f(x) 'receiver_data.csv' using 1:4 via mean_y 
    set label 1 gprintf("Mean = %g", mean_y) at 15, GPVAL_DATA_Y_MAX+10 

    set title "Global Sync Detection"
    set ylabel "Problem Detected"
    plot "receiver_data.csv" using 1:5 title "Global Sync Detect" with points lw 3
